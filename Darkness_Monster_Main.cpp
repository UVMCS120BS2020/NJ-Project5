...
int main() {
    ...
    vector<unique_ptr<Darkness_Monster>> darknessFriends;
    srand(time(NULL));
    for(int i = 0; i < 10; ++i){
        int j = rand() % 2;
        if(j == 1)
            darknessFriends.push_back(make_unique<Closet_Monster>());
        else
            darknessFriends.push_back(make_unique<Under_Bed_Monster>());
    }
